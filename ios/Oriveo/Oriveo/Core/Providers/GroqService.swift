...
final class GroqService: BaseAPIService, ProviderServiceProtocol {
    ...
    func sendMessage(
        ...
        let response: GroqChatCompletionResponse = try await perform(request)
        ...
        let (cost, source) = await MetadataClient.shared.calcCost(
            breakdown: breakdown, modelID: modelID, providerKind: .groq
        )
            ...
            let task = Task {
                do {
                    ...
                    for try await line in bytes.utf8Lines {
                        ...
                        do {
                            chunk = try self.decoder.decode(GroqStreamChunk.self, from: chunkData)
                        } catch {
                            continue
                        }
                        if let usage = chunk.x_groq?.usage ?? chunk.usage { lastUsage = usage }
    ...
    private func buildChatRequest(
        ...
        let resolved = MetadataClient.shared.syncResolveCatalogModel(modelID: modelID, providerKind: .groq)
        ...
        var apiMessages: [GroqChatRequest.Message] = []
        ...
        let reasoningEffort = capabilityIntent.reasoningMode.flatMap { allowedMode in
            ProfileParamsResolver.reasoningMergeParams(
                providerKind: .groq, modelID: modelID,
        ...
        let payload = GroqChatRequest(
            ...
            stream_options: stream ? GroqChatRequest.StreamOptions() : nil,
            reasoning_effort: reasoningEffort,
            messages: apiMessages
        )
        var body = try JSONSerialization.jsonObject(with: JSONEncoder().encode(payload)) as? [String: Any] ?? [:]
        CapabilityRecipeRequestCompiler.apply(to: &body, providerKind: .groq, modelID: modelID, transport: resolved?.transport ?? "", webSearchEnabled: requestOptions.webIntentRequested, reasoningMode: reasoningMode, capabilityPreferences: requestOptions.capabilityPreferences)
        request.httpBody = try encodeChatBody(&body, options: requestOptions, resolved: resolved, finalRequest: request)
        return request
    }

    private static func buildRequestMessage(_ msg: ChatMessage, model: AIModel? = nil) -> GroqChatRequest.Message {
        ...
        var parts: [GroqChatRequest.ContentPart] = [.init(type: "text", text: combinedText)]
        for a in imageAtts {
    ...
    private func buildModels(from remoteModels: [GroqMetadataModel], preferredModelID: String?) async -> [AIModel] {
...
private struct GroqMetadataModel {
    var id: String
}
...
private struct GroqStreamChoice: Decodable {
    var delta: GroqStreamDelta?
    var finish_reason: String?
}

private struct GroqStreamDelta: Decodable {
...
private struct GroqUsage: Decodable {
...
extension GroqService {
    fileprivate static func parseUsage(_ usage: GroqUsage?) -> UsageBreakdown {
