...
final class FireworksService: BaseAPIService, ProviderServiceProtocol {
    private let baseURL = "https://api.fireworks.ai/inference/v1"
    ...
    func sendMessage(
        ...
        let response: FireworksChatCompletionResponse = try await perform(request)
        ...
        let (cost, source) = await MetadataClient.shared.calcCost(
            breakdown: breakdown, modelID: modelID, providerKind: .fireworks
        )
    ...
    private func buildChatRequest(
        ...
        let resolved = MetadataClient.shared.syncResolveCatalogModel(modelID: modelID, providerKind: .fireworks)
        ...
        var apiMessages: [FireworksChatRequest.Message] = []
        ...
        let reasoningEffort = capabilityIntent.reasoningMode.flatMap { allowedMode in
            ProfileParamsResolver.reasoningMergeParams(
                providerKind: .fireworks, modelID: modelID,
                reasoningMode: allowedMode, resolved: resolved
            )?["reasoning_effort"] as? String
        }

        let payload = FireworksChatRequest(
            model: modelID,
            stream: stream ? true : nil,
            stream_options: stream ? FireworksChatRequest.StreamOptions() : nil,
            reasoning_effort: reasoningEffort,
            messages: apiMessages
        )
        var body = try JSONSerialization.jsonObject(with: JSONEncoder().encode(payload)) as? [String: Any] ?? [:]
        CapabilityRecipeRequestCompiler.apply(to: &body, providerKind: .fireworks, modelID: modelID, transport: resolved?.transport ?? "", webSearchEnabled: requestOptions.webIntentRequested, reasoningMode: reasoningMode, capabilityPreferences: requestOptions.capabilityPreferences)
    ...
    private static func buildRequestMessage(_ msg: ChatMessage, model: AIModel? = nil) -> FireworksChatRequest.Message {
        ...
        var parts: [FireworksChatRequest.ContentPart] = [.init(type: "text", text: combinedText)]
        for a in imageAtts {
    ...
    private static func shortModelID(_ fullID: String) -> String {
    ...
    private func displayName(for id: String) -> String {
        Self.shortModelID(id)
    }

    private func buildModels(from remoteModels: [FireworksMetadataModel], preferredModelID: String?) async -> [AIModel] {
        ...
        for remote in remoteModels {
            var model = await CatalogModelBuilder.buildCatalogModel(
                ...
                fallbackName: displayName(for: remote.id),
...
private struct FireworksMetadataModel {
    var id: String
}

private struct FireworksChatRequest: Encodable {
    var model: String
    var stream: Bool?
...
private struct FireworksChatCompletionResponse: Decodable {
    var choices: [Choice]
    var usage: FireworksUsage?
...
private struct FireworksStreamChunk: Decodable {
    var choices: [FireworksStreamChoice]?
    var usage: FireworksUsage?
}

private struct FireworksStreamChoice: Decodable {
    var delta: FireworksStreamDelta?
    var finish_reason: String?
}

private struct FireworksStreamDelta: Decodable {
...
private struct FireworksUsage: Decodable {
...
extension FireworksService {
    fileprivate static func parseUsage(_ usage: FireworksUsage?) -> UsageBreakdown {
