...
struct GrokSubscriptionAuthorizationSheet: View {
    let config: GrokSubscriptionAuthConfig
    let onAuthorized: (GrokSubscriptionTokens) -> Void

    @Environment(\.dismiss) private var dismiss
    @Environment(\.openURL) private var openURL
    @State private var model = GrokSubscriptionAuthorizationModel()
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Text(L10n.tr("Sign in with your Grok subscription", table: .providers))
            ...
            Text(L10n.tr(
                "Authorize with your x.ai account to use your SuperGrok or X Premium quota. No API credits are used.",
                table: .providers
            ))
            .font(.subheadline)
