...
アプリが各プロバイダーと端末から直接やり取りします。会話、ノート、フォルダ、Skills は Room で端末上に
...
flowchart TB
    subgraph ui ["Compose UI"]
        ...
        screens["チャット · ホーム · プロバイダー<br/>ノート · Skills · 設定"]
    end
...
会話 ID ごとの `StreamingSession` を 1 つずつ保持し、それぞれがアプリスコープの
`CoroutineScope(SupervisorJob() + Dispatchers.IO)` を持ちます。チャットから離れても回答は
キャンセルされませんし、`StreamingTokenBuffer` が途中のテキストを定期的に SQLite へ書き出すので、
回答の途中でアプリを終了させても、すでに届いた分が失われることはありません。

**データベースは 1 つではなく 2 つです。** `oriveo.db` には会話、メッセージ、添付ファイル、ノート、
フォルダ、Skills、モデルカタログのキャッシュが入ります。`message_continuations.db` は物理的に別の
...
| 対象 | 保存先 |
|---|---|
| 会話、メッセージ、添付ファイル、ノート、フォルダ、Skills | Room、`oriveo.db` |
...
自分で書き出すバックアップアーカイブは別扱いで、あなたが決めたパスワードを用いて
PBKDF2-HMAC-SHA256 の 600,000 回反復と AES-GCM で暗号化されます。
...
固定して送信時にも再検証し、認証情報を含む平文リクエストは一切拒否し、クロスオリジンやスキームを
変えるリダイレクトも遮断します。
...
> - 失敗は**無言**です。キーを追加しても成功と表示され、モデルピッカーは説明もなく空になります。
> - **OpenAI は使えなくなります**。そのプロバイダーではモデルの手入力が禁止されているためです。
> - Relay のエンドポイントとローカルのモデルサーバーは完全に動作し、唯一無傷の経路になります。
>
...
必要なもの: **JDK 17 以降**と Android SDK。ビルドには AGP 9.3、Gradle 9.5、Kotlin 2.3 を使うため、
...
319 ファイルにおよそ 3,000 のユニットテストがあり、JUnit 4、MockK、Turbine、
`kotlinx-coroutines-test`、Ktor のモックエンジンを使っています。カバレッジが最も厚いのは、間違いの
代償が最も大きいところです。プロバイダーごとのリクエストの形、SSE のパース、トランスポートの選択、
relay のプローブとセキュリティモード、機能レシピの実行、カタログのキャッシュとコントラクトバージョン
の扱い、Room の永続化、バックアップの往復です。

> [!IMPORTANT]
> およそ 38 のスイートが作業ディレクトリから上へたどって `shared/` を見つけ、コントラクトの
> フィクスチャを読み込みます。そのため**テストが通るのはリポジトリ全体をチェックアウトしたときだけ**
> です。`android/` だけをコピーしても動きません。
...
16 言語です。`values/`（英語、ソース）に加えて 15 個の `values-*` ディレクトリがあり、それぞれおよそ
1,700 の文字列を持ち、すべてのロケールが同一のキー集合を保持しています。アプリ内での言語切り替えは
