...
직접 내보내는 백업 아카이브는 별도로 암호화되며, 반복 600,000회의 PBKDF2-HMAC-SHA256과 AES-GCM을
쓰고 사용자가 정한 비밀번호를 사용합니다.
...
319개 파일에 걸쳐 단위 테스트 약 3,000개가 있으며, JUnit 4, MockK, Turbine,
`kotlinx-coroutines-test`, Ktor의 mock 엔진을 씁니다. 실수의 대가가 가장 큰 곳에 커버리지가 가장
두텁습니다. 공급자별 요청 형태, SSE 파싱, transport 선택, 릴레이 프로빙과 보안 모드, 기능 레시피
...
16개 언어입니다. `values/`(영어, 소스)와 열다섯 개의 `values-*` 디렉터리로 이루어지며, 각각 약
1,700개 문자열을 담고 모든 로케일이 동일한 키 집합을 가집니다. 앱 내 언어 전환은
