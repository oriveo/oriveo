...
`recorded/` 아래의 `.sse` 파일은 **실제로 캡처한 업스트림 트래픽**이며 바이트 하나 건드리지 않고
그대로 둡니다. 나머지는 특정 파싱 경로를 고정하려고 손으로 쓴 fixture입니다. 이 구분은 중요합니다.
손으로 쓴 mock은 공급자가 이렇게 동작할 것이라고 당신이 믿었던 바를 담지만, 녹화된 스트림은 공급자가
실제로 한 일을 담습니다 — 그날 화요일에 보내온 그 망가진 청크까지 포함해서. 공급자 프로토콜 수정에
테스트가 필요할 때는 녹화를 우선하세요.
...
스위트는 Gradle 모듈에서 `../../shared`를 해석하며, 웹 스위트는 워크스페이스를 기준으로 해석합니다.
따라서 셋 다 저장소의 전체 체크아웃이 필요합니다.
