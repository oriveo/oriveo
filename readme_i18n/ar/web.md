...
والمعالج ليس واحدا: فبث المحادثة، وموجّه relay، وتوليد الصور، وقائمة النماذج، والتحقق من المفتاح،
وتبادلا تسجيل الدخول بالجهاز لدى Grok وCodex تبلغ في مجموعها اثني عشر ملف مسار. والتحقق من المفتاح
...
فقط؛ وكل مفتاح موثّق هناك. وثمة متغيرات قليلة يقرأها الكود وليست في ذلك الملف:
`BACKEND_URL` (توأم لـ `NEXT_PUBLIC_BACKEND_URL` على جانب الخادم فقط)، و`NEXT_PUBLIC_LIBRARY_ENABLED`،
و`ORIVEO_DESKTOP`، و`NEXT_DIST_DIR`.
