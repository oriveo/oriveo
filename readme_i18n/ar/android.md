...
محادثة داخل `ConcurrentHashMap`، وتعمل كل واحدة منها كـ `Job` خاص بها فوق
`CoroutineScope(SupervisorJob() + Dispatchers.IO)` واحد بنطاق التطبيق — والمشرف هو بيت القصيد،
فتعثّر بث واحد لا يُسقط البقية. فالخروج من شاشة المحادثة لا يلغي الإجابة، و`ChatRepository` يفرّغ
النص الجزئي إلى SQLite كلما أبلغ `StreamingTokenBuffer` بأن ما تراكم يكفي (4000 محرف أو 60 ثانية)،
فإغلاق التطبيق في منتصف الإجابة لا يفقد ما وصل بالفعل.
...
والأرشيف الذي تصدّره أنت ملف zip يحمل `data.json` مع ملفات المرفقات. وكلمة المرور التي تختارها
تحمي **مفاتيح API للمزودين وحدها** داخله: فهي تُشفَّر بـ PBKDF2-HMAC-SHA256 على 600,000 دورة
وAES-GCM وتُخزَّن كحقل واحد في `data.json`. أما المحادثات والرسائل والملاحظات والمجلدات والمهارات
والتفضيلات والمرفقات فتُكتب بصيغة JSON صريحة وملفات صريحة في كل الأحوال، فتعامَل مع الأرشيف على أنه
مقروء لكل من يحصل على الملف. صدّر بدون المفاتيح إن كنت تريد سجلّك وحده.
...
ويثبّت مجموعة العناوين المحلولة ضد إعادة ربط DNS، ويعيد التحقق منها عند الإرسال. وهو يرفض أي طلب
بنص صريح يحمل بيانات اعتماد. أما إعادة التوجيه فلا تُتَّبع إطلاقا في عميلي الاكتشاف والمحركات
المحلية، وتثبيت العناوين ذاك هو خط الدفاع الأخير.
...
> - وتعرض شاشة تفاصيل المزود شريطا يقول «تعذّر تحميل النماذج الرسمية»، لكن إضافة المفتاح تظل
>   تُبلغ عن النجاح وقائمة اختيار النماذج تكون فارغة ببساطة؛
> - و**يصبح OpenAI غير قابل للاستخدام**، لأن الإدخال اليدوي للنماذج محجوب لدى ذلك المزود؛
> - أما نقاط خدمة الترحيل (Relay) وخوادم النماذج المحلية فتظل تعمل بالكامل، وهي المسار السليم الوحيد.
>
...
المتطلبات: **JDK 21** وAndroid SDK. يستخدم البناء AGP 9.3 وGradle 9.5 وKotlin 2.3، لذا يجب
...
نحو 3,000 اختبار وحدة موزعة على 318 ملفا، باستخدام JUnit 4 وMockK وRobolectric
...
> [!IMPORTANT]
> تحمّل نحو 38 مجموعة اختبار نسخ العقود المرجعية بالصعود من مجلد العمل حتى تجد `shared/`، لذا **لا
> تنجح الاختبارات إلا في نسخة كاملة من المستودع** — نسخ مجلد `android/` وحده لن ينفع.
...
ست عشرة لغة: مجلد `values/` (الإنجليزية، وهي المصدر) إضافة إلى خمسة عشر مجلد لغة — إلى جانب
`values-night` الذي لا يحمل أي نصوص — بنحو 1,340 نصا لكل منها، وكل لغة تحمل مجموعة المفاتيح نفسها
بالضبط. ويمر تبديل اللغة داخل التطبيق عبر
