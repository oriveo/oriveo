<div align="center">

# العقود المشتركة

**تعريف واحد لكيفية التحدث إلى مزود نماذج، يختبره العملاء الثلاثة جميعا.**

<a href="../../LICENSE"><img alt="الرخصة AGPL-3.0-or-later" src="https://img.shields.io/badge/license-AGPL--3.0--or--later-8B5CF6?style=flat-square&labelColor=black"></a>
<img alt="حزمة Swift 6.1" src="https://img.shields.io/badge/Swift-6.1-A78BFA?style=flat-square&labelColor=black&logo=swift&logoColor=white">
<img alt="عقود JSON" src="https://img.shields.io/badge/contracts-JSON-A78BFA?style=flat-square&labelColor=black">

<sub>

<a href="../../shared/README.md">English</a> ·
**العربية** ·
<a href="../de/shared.md">Deutsch</a> ·
<a href="../es/shared.md">Español</a> ·
<a href="../fr/shared.md">Français</a> ·
<a href="../hi/shared.md">हिन्दी</a> ·
<a href="../id/shared.md">Indonesia</a> ·
<a href="../ja/shared.md">日本語</a> ·
<a href="../ko/shared.md">한국어</a> ·
<a href="../pt-BR/shared.md">Português</a> ·
<a href="../ru/shared.md">Русский</a> ·
<a href="../th/shared.md">ไทย</a> ·
<a href="../tr/shared.md">Türkçe</a> ·
<a href="../vi/shared.md">Tiếng Việt</a> ·
<a href="../zh-Hans/shared.md">简体中文</a> ·
<a href="../zh-Hant/shared.md">繁體中文</a>

</sub>

</div>

---

ثلاثة عملاء ينفّذ كل منهم «نادِ المزود» على حدة سينحرفون عن بعضهم. سينحرفون بهدوء، في اتجاه أيّهم
اختبره أحدهم آخر مرة، وسيظهر الانحراف على شكل علة تتكرر على منصة واحدة دون البقية.

ومجلد `shared/` هو الجواب على ذلك: يُكتب السلوك مرة واحدة كبيانات، وتختبر مجموعة اختبارات كل عميل
نفسها على الملفات ذاتها. فالخصوصية التي تعيش في تلك البيانات تُصلَح مرة واحدة. أما التي تعيش في
محلّل فتلتقطها ثلاث مجموعات اختبار في الوقت نفسه، بدل أن تُشحن على منصتين وتكسر الثالثة.

```mermaid
flowchart LR
    subgraph contracts ["shared/"]
        direction TB
        recipes["capabilityrecipe<br/>كيف يُشكَّل الطلب"]
        models["model-contracts<br/>ما يجوز للعميل فعله"]
        fixtures["test-fixtures<br/>حركة مسجّلة من المصدر"]
        kit["OriveoProviderKit<br/>نواة الاتصال بلغة Swift"]
    end

    iosT["مجموعات اختبار iOS"]
    andT["مجموعات اختبار Android"]
    webT["مجموعات اختبار الويب"]

    recipes & models & fixtures --> iosT & andT & webT
    kit --> iosT
```

## capabilityrecipe

سجل الوصفات. لمزود ونقل وقدرة معينة — البحث على الويب، ومستوى جهد الاستدلال، وتوليد الصور — يقول
بالضبط أي مؤشرات JSON تُكتب في الطلب الصادر، وكيف تُقرأ الإجابة عائدة.

وهذا ما يجعل نموذجا صدر اليوم يعمل دون تحديث العميل، وهو سبب ألا يخمّن أي عميل قدرة من اسم نموذج.
يحمل `capability_runtime.v1.json` الوصفات نفسها؛ بينما يعرّف `capability_result_definitions.v1.json`
و`capability_custom_controls.v2.json` كيف تُفسَّر النتائج وعناصر التحكم التي يراها المستخدم.

وتعلن كل وصفة قيمة `executionKind` — `request_overlay` أو `server_tool` أو `client_tool_loop` أو
`endpoint_route` أو `model_route` أو `external_connector` أو `unavailable` — ويتحقق مترجم كل عميل من
مطابقة الوصفة للمزود والقدرة والنقل قبل تطبيقها، ويرفض بسبب مسمّى بدل إرسال طلب لم يراجعه أحد.
والقائمة مجموعة مغلقة: فأي وصفة تسمّي شيئا آخر تُرفَض ولا يُخمَّن معناها.

## model-contracts

نسخ JSON مرجعية تثبّت السلوك عبر العملاء: كيف يجب أن يبدو الطلب لمزود وقدرة معينة، وكيف تُحلّ
معاملات التوليد وكيف تتراكب التجاوزات، وأي حالات قدرة يجوز للعميل عرضها، وكيف يُستهلك فهرس
النماذج وأدلته.

وتحمّل اختبارات كل عميل هذه الملفات مباشرة، فأي تغيير هنا هو تغيير في العملاء الثلاثة دفعة واحدة.

## test-fixtures

بيانات اختبار ذهبية: حركة نداء أدوات مسجّلة من المصدر، وتوجيه relay، والتحقق من صحة النماذج
المدخَلة، وتصنيف العناوين المحلية، وسيناريوهات الفهرس والإعدادات المحمولة، ولقطات لحقائق النماذج
وأدلة القدرات، وسيناريوهات المحركات المحلية.

وملفات `.sse` الموجودة تحت `provider-toolcall/recorded/` هي **حركة حقيقية ملتقطة من المصدر**،
محفوظة بايتا ببايت كما وصلت — أُسقِطت ترويسات الاستجابة وحدها، ولم تحمل الأجسام مفتاحا قطّ. أما
ملفات `.sse` الموجودة مباشرة في `provider-toolcall/` فنسخ مرجعية مكتوبة يدويا تثبّت مسار تحليل
بعينه. والفرق مهم: فالمحاكاة المكتوبة
يدويا تشفّر ما اعتقدتَ أن المزود يفعله، بينما يشفّر التسجيل ما فعله فعلا، بما في ذلك ذلك الجزء
المشوّه الذي أرسله يوم الثلاثاء. وحين يحتاج إصلاح في بروتوكول مزود إلى اختبار، فضّل تسجيلا.

ويقول `$comment` النسخة المرجعية، أو ملف `expected.json` الموجود بجانبها، ما تثبّته المدخلات من
حولها. اقرأ ذلك قبل إضافة حالة جديدة.

## OriveoProviderKit

حزمة Swift تحمل نواة بروتوكول الاتصال بالمزودين: تجميع أسطر SSE، وتحليل الأجزاء المتوافقة مع
OpenAI، والتجميع القائم على الأحداث لبروتوكولات Responses وAnthropic Messages وGemini، وبناء الطلبات
بمعزل عن النقل، وترجمة الوصفات وحرّاس تنفيذها، وترميز أسماء الأدوات، وحجب بيانات الاعتماد، وتصنيف
أخطاء المصدر، وتحليل وسوم التفكير، واستخراج مسارات JSON أثناء البث، وسياسة صريحة لإعادة التوجيه في
`URLSession`، وملفات خصوصيات كل مورّد.

ونطاقها مرسوم بضيق مقصود. **داخله:** معرفة الاتصال المعتمدة على Foundation وحدها. **خارجه:**
نماذج التطبيق والواجهة وقاعدة البيانات والقياس والتوطين. ولا تعتمد الحزمة على شيء خارج المكتبة
القياسية وFoundation، ويحتفظ كل عميل من عملاء Apple بربط رقيق حولها، فيكون لسلوك الاتصال تنفيذ واحد
بالضبط.

وهي تنفّذ مسار الطلب والبث كاملا على منصات Apple. ويربط تطبيق iOS حاليا جزءا منها — مجمّعات البث،
وملفات خصوصيات الاتصال، ومرمّز أسماء الأدوات، ومصنّفات الأخطاء — ويحتفظ ببناة الطلبات الخاصة به؛
وعميل macOS قيد التطوير هو المستهلك الثاني، ولهذا يعيش مترجم الوصفات وباني الطلبات المستقل عن النقل
هنا لا داخل تطبيق واحد. وتغطي المجموعة أدناه الأجزاء التي يتشاركها كل مستهلك: تقسيم SSE، والتجميع
المتوافق مع OpenAI، ومرمّز أسماء الأدوات، وسياسة إعادة التوجيه.

```bash
cd shared/OriveoProviderKit && swift build && swift test
```

- المنصات: iOS 18+ وmacOS 15+ · `swift-tools-version: 6.1`
- يحمل `ProviderWireProfile` ما تبقى من خصوصيات كل مورّد مما لا يزال يحتاجه مجمّع واحد متوافق مع
  OpenAI — أين يصل نص الاستدلال، وأين تعيش أعداد الرموز المخزّنة مؤقتا، وهل تتضمن رموز الموجّه
  إصابات التخزين المؤقت أصلا. وهو يصف *كيف تصل البايتات*، لا *ما يستطيع نموذج فعله*؛ فتلك مهمة
  الوصفات.

## العمل على هذه الملفات

أي تغيير هنا تغيير في كل عميل. شغّل مجموعات اختبار العقود لكل عميل يقرأ الملف الذي مسسته، لا
المجموعة التي تصادف أنك تعمل فيها فقط:

من جذر المستودع:

```bash
(cd web && npm run test:run)
(cd shared/OriveoProviderKit && swift test)
# plus the iOS and Android suites — see their READMEs
```

وتحدّد مجموعات اختبار iOS موقع هذا المجلد بالصعود من ملف الاختبار حتى تجد `shared/`؛ أما مجموعات
Android فتصعد من مجلد العمل بالطريقة نفسها، ومجموعات الويب تحلّه نسبة إلى مساحة العمل. ولذلك تتطلب
جميعها نسخة كاملة من المستودع.

وقبل فتح طلب سحب، اقرأ [CONTRIBUTING.md](../../CONTRIBUTING.md).

## الترخيص

[AGPL-3.0-or-later](../../LICENSE).
