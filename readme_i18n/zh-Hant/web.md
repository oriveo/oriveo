...
handler 不只一個：聊天串流、relay 轉送器、圖片生成、模型清單、Key 驗證，以及 Grok 與 Codex 的裝置
...
項目；每一個鍵在那裡都有說明。另外有幾個程式會讀、但不在那個檔案裡的變數：`BACKEND_URL`
（`NEXT_PUBLIC_BACKEND_URL` 的伺服器端專用孿生版）、`NEXT_PUBLIC_LIBRARY_ENABLED`、`ORIVEO_DESKTOP`
與 `NEXT_DIST_DIR`。
