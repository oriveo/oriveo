...
Oriveo Community Edition — это открытый AI-чат в модели BYOK, со своим ключом, для iOS, Android и
веба, и нативный клиент для macOS в разработке. Он для тех, кто предпочитает платить провайдеру
модели напрямую, а не подписку тому, кто стоит перед ним: вы указываете API-ключи, которые у вас уже
есть, и клиент обращается с ними к провайдеру. Это делает его local-first и мультимодельной
альтернативой размещённому тарифу ChatGPT или Claude — без аккаунта Oriveo, без подписки, без чего бы
то ни было, что отчитывается перед нами, и с веб-клиентом, который вы можете разместить сами.
...
| **Клиенты** | iOS (SwiftUI) · Android (Jetpack Compose) · Web (Next.js) · macOS в разработке |
| **Языки интерфейса** | 16 |
| **Нужен ли аккаунт** | Нет |
| **Запросы, которые он делает от своего имени** | Одно дело, в двух запросах: каталог моделей только на чтение, без ключа и без добавленного нами идентификатора |
...
- **Единственное, что он забирает.** Приложение читает публичный каталог моделей, чтобы вышедшая
  сегодня модель работала без обновления приложения. Оба его запроса только на чтение и не несут ни
  ключа, ни добавленного нами идентификатора, а веб-клиент и Android-клиент можно направить на свой
  хост.
...
- **Локальные серверы моделей** — llama.cpp, Ollama, LM Studio, vLLM, Open WebUI; iOS и Android
  находят их в локальной сети через mDNS
...
- **Skills** — переиспользуемые системные промпты со своей моделью, настройкой рассуждений и
  справочными документами
- **Заметки и папки** — сохраните ответ как заметку, наведите порядок в разговорах, ищите и по тем, и
  по другим
- **Перекрёстная проверка** — отдайте ответ второй модели на разбор и держите оба вместе
...
| Аналитика и отчёты о сбоях | Нет. В веб-бандл входит Sentry, молчащий, пока вы не зададите собственный DSN | Да |
...
| **macOS** | В разработке, появится в ближайшие месяцы | [macos/README.md](macos.md) |
| **Shared** | Контракты, записанные фикстуры и Swift-ядро протокола | [shared/README.md](shared.md) |

## Начало работы

Готовых сборок здесь нет — ни APK, ни `.ipa`. Community Edition — это исходный код,
...
Требуется Node 22.22 или новее (см. [`web/.nvmrc`](../../web/.nvmrc)).
...
- **Два наших собственных запроса:** каталог моделей только на чтение, читаемый двумя вызовами —
  один про то, как следует обращаться к каждой модели, второй про факты об отдельных моделях, который
  iOS читает только после входа по подписке, — чтобы вышедшая сегодня модель работала без новой
  сборки. Ни один из них не несёт ни ключа, ни разговора,
  ни добавленного нами идентификатора. Веб-клиент (`NEXT_PUBLIC_BACKEND_URL`) и сборку Android
  (`-PORIVEO_METADATA_BASE_URL`) можно направить на свой хост; на iOS это переопределение — лишь
  удобство для Debug-сборки.
...
Клиент — да. Он открыт под AGPL-3.0-or-later, подписываться не на что, и ни одна его часть не спрятана
за оплатой. Вы платите провайдеру модели по его собственному прайс-листу за те запросы, которые
...
которым управляет Oriveo. Единственное, что Oriveo забирает от своего имени, — это публичный каталог
моделей, двумя запросами только на чтение, которые не несут ни ключа, ни разговора, ни добавленного
нами идентификатора.
...
Gemini — llama.cpp, Ollama, LM Studio, vLLM, Open WebUI или что угодно ещё, говорящее на одном из
этих протоколов. Клиенты iOS и Android умеют находить такой сервер в локальной сети через mDNS, а
веб-клиент предлагает для каждого движка его обычный адрес и проверяет его. Локальный HTTP не
использует никаких учётных данных и никогда не покидает вашу сеть.

</details>

<details>
<summary><b>Можно ли поднять всё это самому?</b></summary>

<br>

Да. Веб-клиент — это приложение Next.js, которое вы собираете и отдаёте со своей машины; это
единственная часть проекта, у которой вообще есть серверная сторона, и она не хранит ни ключей, ни
сообщений. Направьте его на сервер модели на своём железе, и ни один запрос не покинет вашу сеть.
Каталог моделей тоже можно разместить у себя: задайте веб-сборке свой `NEXT_PUBLIC_BACKEND_URL` или
сборке Android `-PORIVEO_METADATA_BASE_URL` — и в приложении не останется ничего, что тянулось бы за
пределы вашей сети.
...
Нативный клиент для macOS в разработке и выйдет в ближайшие месяцы; `macos/` — то место, где он
появится. А пока веб-клиент неплохо работает как настольное приложение в любом браузере, а сборка для
iOS запускается на Mac с Apple silicon прямо из Xcode. Swift-пакет, который разговаривает с
провайдерами, уже объявляет macOS 15 поддерживаемой платформой, так что протокольный слой, нужный
Mac-клиенту, уже написан и уже под тестами. См. [macos/README.md](macos.md).
