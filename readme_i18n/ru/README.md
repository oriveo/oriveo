...
Без аккаунта, без подписки, без нашего сервера между вами и моделью.
...
Oriveo, нет подписки и нет аналитики.

Он нативно работает с **15 провайдерами моделей** — OpenAI, Anthropic, Google Gemini, OpenRouter,
DeepSeek, Grok, Mistral, Groq, Together AI, Fireworks AI, MiniMax, Z.ai, Qwen, Kimi и SiliconFlow —
плюс с **любым endpoint, совместимым с OpenAI, Anthropic или Gemini**, на который вы его направите,
включая llama.cpp, Ollama, LM Studio или vLLM, запущенные на вашей собственной машине.
...
| **Запросы, которые он делает от своего имени** | Один: каталог моделей только на чтение, без приложенного ключа и без идентификатора |
| **Лицензия** | AGPL-3.0-or-later |

## Зачем он нужен

Клиент чата не должен стоять между вами и моделью, за которую вы платите.
...
- **Одно поведение, три клиента.** То, как формируется запрос для конкретного провайдера,
  транспорта и возможности, определено один раз в [`shared/`](shared.md), и все три клиента
  проверяются по одним и тем же JSON-фикстурам. Причуда провайдера чинится один раз, а не три.
- **Честность про единственный собственный запрос.** Приложение забирает публичный каталог моделей,
...
- **Relay** — любой endpoint, совместимый с OpenAI, Anthropic или Gemini, в том числе в вашей
  локальной сети
- **Локальные серверы моделей** — llama.cpp, Ollama, LM Studio, vLLM, с обнаружением в локальной
  сети
...
| Заметки, папки, skills, вложения | Да, без ограничений | Да |
...
| Аналитика и отчёты о сбоях | Нет | Да |

Сборки Community Edition используют префикс идентификатора `ai.oriveo.community`, поэтому такая
сборка может стоять рядом со сборкой из магазина, не разделяя с ней ни keychain, ни канал
обновлений, ни локальные данные. Что эта редакция принимает, а что нет, записано в
...
Единственная асимметрия, о которой стоит знать, — веб-клиент. API провайдеров не отдают
CORS-заголовки, поэтому браузер не может обратиться к ним напрямую; из-за этого запросы к 15
официальным провайдерам идут через route handler Next.js, работающий на той машине, которая отдаёт
приложение, — на вашей собственной, когда вы запускаете его локально. У клиентов iOS и Android
такого ограничения нет, и они идут прямо к провайдеру. Relay-endpoint'ы в вашей собственной сети
браузер тоже вызывает напрямую.
...
Требуется JDK 17 или новее и Android SDK. Сборка использует AGP 9.3, Gradle 9.5 и Kotlin 2.3,
поэтому Android Studio должна быть версией, способной их синхронизировать; из командной строки нужны
только JDK и SDK.
...
- **Ключи провайдеров** хранятся штатным средством самой платформы — iOS Keychain, Android Keystore
  (`EncryptedSharedPreferences`) или IndexedDB браузера — и используются только для обращения к тому
  провайдеру, которому принадлежат. В вебе они хранятся без шифрования: это та же модель, которую
  обычно используют браузерные BYOK-клиенты; за самой сильной гарантией идите в клиент iOS или
  Android.
- **Разговоры, заметки, папки, skills и вложения** хранятся на устройстве. Ничего никуда не
  загружается.
- **Ни аккаунта, ни аналитики, ни отчётов о сбоях.** Некуда входить, и ничто не звонит домой.
- **На iOS и Android запросы чата идут прямо с устройства к провайдеру.** В вебе они проходят через
  сервер Next.js, который отдаёт приложение, потому что API провайдеров не разрешают прямые вызовы
  из браузера; этот сервер не сохраняет ни ключи, ни сообщения, а когда вы запускаете приложение
  локально — это ваша собственная машина.
...
Нет. На iOS и Android клиент вызывает endpoint провайдера напрямую. В вебе запрос идёт через сервер
Next.js, который отдаёт приложение — вашу собственную машину, когда вы запускаете его локально, —
потому что браузеры не могут обращаться к API провайдеров напрямую. Ни один из этих путей не
проходит через сервер, которым управляет Oriveo. Единственный запрос, который Oriveo делает от
своего имени, — это чтение публичного каталога моделей: без ключа, без разговора и без
идентификатора.
...
протоколов. Клиенты Android и веб умеют ещё и находить такой сервер в локальной сети. Локальный HTTP
не использует никаких учётных данных и никогда не покидает вашу сеть.
...
биллинга, без аналитики. Полное сравнение — в разделе
...
браузере, а сборка для iOS запускается на Mac с Apple silicon.
