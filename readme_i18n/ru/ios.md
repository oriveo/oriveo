...
заметок лежат в базе SQLite на устройстве; блобы вложений — файлы рядом с ней; навыки, настройки,
...
провайдеров, заметки, навыки — на SwiftUI. Разделение существует потому, что ленте, которая
...
PBKDF2-HMAC-SHA256 за 600 000 итераций). Разговоры, заметки, навыки и настройки в любом случае лежат
...
(Designed for iPad)**. Mac Catalyst не включён — проект никогда его не подключает, а
`TARGETED_DEVICE_FAMILY` остаётся `1,2`, — так что это не Mac-приложение, а iOS-приложение в среде
совместимости с iPad, и пути, существующие только на устройстве, вроде съёмки на камеру, ведут себя
так, как ведут себя на Mac.
...
Testing](https://github.com/swiftlang/swift-testing) плюс 76 на XCTest, в 275 файлах. Он покрывает
