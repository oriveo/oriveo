...
`endpoint_route`, `model_route`, `external_connector`, `unavailable` — и компилятор каждого клиента
проверяет, что рецепт соответствует провайдеру, возможности и транспорту, прежде чем его применить,
отклоняя с названной причиной, а не отправляя запрос, который никто не проверял. Список закрыт:
рецепт, называющий что-либо ещё, отклоняется, а не додумывается.
...
Эталонные тестовые данные: записанный трафик tool call от upstream, маршрутизация relay, проверка
форм, классификация локальных адресов, сценарии каталога и переносимой конфигурации, снимки
model facts и свидетельств о возможностях, сценарии локальных движков.

Файлы `.sse` внутри `provider-toolcall/recorded/` — это **реально захваченный трафик upstream**,
сохранённый байт в байт таким, каким пришёл: выброшены только заголовки ответа, а в телах никогда не
было ключа. Файлы `.sse`, лежащие прямо в `provider-toolcall/`, — написанные вручную фикстуры,
прибивающие конкретный путь разбора. Различие важно:
написанный вручную мок кодирует то, что вы думали о поведении провайдера, а запись кодирует то, что он
на самом деле сделал, включая битый чанк, который он прислал в тот вторник. Когда исправлению
протокола провайдера нужен тест, предпочитайте запись.

`$comment` фикстуры или лежащий рядом манифест `expected.json` говорят, что именно прибивают
окружающие записи. Прочитайте это, прежде чем добавлять случай.

## OriveoProviderKit

Swift-пакет с ядром сетевого протокола провайдеров: сборка строк SSE, разбор OpenAI-совместимых
чанков, событийная сборка для протоколов Responses / Anthropic Messages / Gemini, построение запросов
без привязки к транспорту, компиляция рецептов и её защитные проверки при исполнении, кодирование
имён tool, сокрытие учётных данных, классификация ошибок upstream, разбор тегов thinking, извлечение
JSON-путей на потоке, явная политика редиректов `URLSession` и профили причуд по каждому провайдеру.

Его границы намеренно проведены узко. **Внутри:** знание о сети, опирающееся только на Foundation.
**Снаружи:** модели приложения, UI, база данных, телеметрия, локализация. Пакет не зависит ни от чего
за пределами стандартной библиотеки и Foundation, и каждый Apple-клиент держит вокруг него тонкую
обвязку, чтобы у сетевого поведения была ровно одна реализация.

Он реализует весь путь запроса и стриминга для платформ Apple. Приложение iOS сейчас подключает лишь
часть — сборщики потока, сетевые профили, кодек имён tool и классификаторы ошибок — и держит свои
построители запросов; второй потребитель — разрабатываемый клиент для macOS, и именно поэтому
компилятор рецептов и построитель запросов без привязки к транспорту живут здесь, а не внутри одного
приложения. Набор ниже покрывает то, что общее у всех потребителей: разбиение SSE,
OpenAI-совместимую сборку, кодек имён tool и политику редиректов.
...
Android-наборы поднимаются точно так же от рабочей директории; веб-наборы резолвят её относительно
workspace. Поэтому всем им нужна полная копия репозитория.
