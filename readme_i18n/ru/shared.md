...
проверяется по одним и тем же файлам. Причуда провайдера чинится один раз. Изменение контракта роняет
три набора одновременно, вместо того чтобы уехать в релиз на двух платформах и сломать третью.
...
Файлы `.sse` — это **реально захваченный трафик upstream**, и они остаются нетронутыми, байт в байт.
Написанный вручную мок кодирует то, что вы думали о поведении провайдера; записанный поток кодирует
то, что он на самом деле сделал, включая битый чанк, который он прислал в тот вторник. Когда
исправлению протокола провайдера нужен тест, запись стоит больше мока.
...
И iOS-, и Android-наборы находят эту директорию, поднимаясь вверх от файла теста, пока не встретят
`shared/`, а веб-наборы резолвят её относительно workspace. Поэтому всем им нужна полная копия
репозитория.
