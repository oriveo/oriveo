...
проверка ключа и обмены при входе по устройству для Grok и Codex дают в сумме двенадцать файлов
...
то, что вам нужно; каждый ключ там описан. Несколько переменных, которые читает код, в этот файл не
попали: `BACKEND_URL` (чисто серверный двойник `NEXT_PUBLIC_BACKEND_URL`),
`NEXT_PUBLIC_LIBRARY_ENABLED`, `ORIVEO_DESKTOP` и `NEXT_DIST_DIR`.
