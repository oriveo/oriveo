...
навыки и ваши ключи провайдеров живут в собственном хранилище браузера. Ни аккаунта, ни входа.
...
Требуется Node 22.22.2 или более поздняя 22.x (см. [`.nvmrc`](../../web/.nvmrc)); поле `engines` — `^22.22.2`, так что Node 23+ не поддерживается. npm идёт вместе с ним; другой
...
flowchart TB
    subgraph app ["apps/app — приложение Next.js"]
        direction LR
        routes["App Router<br/>чат · заметки · провайдеры · навыки · настройки"]
...
| `npm run test` | vitest в режиме watch, по одному наблюдателю на workspace — лучше запускать внутри одного workspace |
...
|---|---|---|
| [Next.js](https://nextjs.org) | 16.3.3 | App Router, route handlers, сборка |
| [React](https://react.dev) | 19.2.8 | интерфейс |
| [vitest](https://vitest.dev) | 4.1.11 | запуск тестов |
| [zustand](https://zustand.docs.pmnd.rs) | 5.0.15 | состояние на клиенте |
| [next-intl](https://next-intl.dev) | 4.14.1 | локализация |
| [@sentry/nextjs](https://docs.sentry.io/platforms/javascript/guides/nextjs/) | 10.72.0 | отчёты об ошибках, без DSN бездействует |

Точные версии всех зависимостей зафиксированы в `package-lock.json`.
...
> [!IMPORTANT]
> Более тридцати наборов резолвят контрактные фикстуры в `shared/` относительно рабочей директории,
> поэтому **тесты проходят только в полной копии репозитория**, запущенные из того workspace,
> которому принадлежат, — если скопировать наружу один `web/`, работать не будет.
