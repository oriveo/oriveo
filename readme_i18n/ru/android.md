...
skills хранятся на устройстве в Room; API-ключи шифруются ключом, который держит Android Keystore.
...
flowchart TB
    subgraph ui ["Compose UI"]
        ...
        screens["Чат · Главная · Провайдеры<br/>Заметки · Skills · Настройки"]
    end
...
разговора в `ConcurrentHashMap`, у каждой свой `CoroutineScope(SupervisorJob() + Dispatchers.IO)` в
скоупе приложения. Уход с экрана чата не отменяет ответ, а `StreamingTokenBuffer` периодически
сбрасывает частичный текст в SQLite, так что убийство приложения посреди ответа не теряет то, что
уже пришло.

**Две базы данных, а не одна.** `oriveo.db` хранит разговоры, сообщения, вложения, заметки, папки,
skills и кэш каталога моделей. `message_continuations.db` — физически отдельный файл с непрозрачным
...
| Что | Где |
|---|---|
| Разговоры, сообщения, вложения, заметки, папки, skills | Room, `oriveo.db` |
...
Архивы резервных копий, которые вы экспортируете сами, шифруются отдельно: PBKDF2-HMAC-SHA256 с
600 000 итераций и AES-GCM, паролем, который выбираете вы.
...
DNS rebinding и перепроверяет его в момент отправки, отказывает любому запросу в открытом виде,
несущему учётные данные, и блокирует редиректы на другой origin или со сменой схемы.
...
> - отказ **происходит молча**. Добавление ключа по-прежнему сообщает об успехе, а список выбора
>   моделей просто пуст и ничего не объясняет;
...
Требования: **JDK 17 или новее** и Android SDK. Сборка использует AGP 9.3, Gradle 9.5 и Kotlin 2.3,
поэтому Android Studio должна быть версией, способной синхронизировать AGP 9.3; из командной строки
нужны только JDK и SDK.
...
Примерно 3 000 юнит-тестов в 319 файлах, на JUnit 4, MockK, Turbine, `kotlinx-coroutines-test` и
mock engine из Ktor. Покрытие плотнее всего там, где ошибки обходятся дороже всего: форма запроса
...
> [!IMPORTANT]
> Около 38 наборов загружают контрактные фикстуры из `shared/`, поднимаясь вверх от рабочей
> директории, поэтому **тесты проходят только в полной копии репозитория** — если скопировать наружу
> один `android/`, работать не будет.
...
Шестнадцать языков: `values/` (английский, исходный) плюс пятнадцать директорий `values-*`, примерно
по 1 700 строк в каждой, и во всех локалях набор ключей одинаковый. Смена языка внутри приложения
