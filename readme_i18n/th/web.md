...
การตรวจสอบคีย์ และการแลกเปลี่ยน device login ของ Grok กับ Codex ซึ่งรวมกันแล้วเป็นไฟล์ route
...
มีตัวแปรอีกไม่กี่ตัวที่โค้ดอ่านแต่ไม่ได้อยู่ในไฟล์นั้น ได้แก่ `BACKEND_URL`
(ฝาแฝดฝั่งเซิร์ฟเวอร์อย่างเดียวของ `NEXT_PUBLIC_BACKEND_URL`), `NEXT_PUBLIC_LIBRARY_ENABLED`,
`ORIVEO_DESKTOP` และ `NEXT_DIST_DIR`
