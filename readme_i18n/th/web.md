...
โฟลเดอร์ ทักษะ และคีย์ผู้ให้บริการของคุณ อยู่ในที่จัดเก็บข้อมูลของเบราว์เซอร์เอง
...
ต้องใช้ Node 22.22.2 หรือ 22.x ที่ใหม่กว่า (ดู [`.nvmrc`](../../web/.nvmrc)) ฟิลด์ `engines`
ระบุไว้เป็น `^22.22.2` ดังนั้น Node 23+ จึงไม่รองรับ npm มาพร้อมกับ Node อยู่แล้ว
...
handler ไม่ได้มีแค่ตัวเดียว ได้แก่ การสตรีมแชท ตัวส่งต่อของบริการรีเลย์ (Relay) การสร้างภาพ รายชื่อโมเดล
...
flowchart TB
    subgraph app ["apps/app — แอปพลิเคชัน Next.js"]
        direction LR
        routes["App Router<br/>แชท · โน้ต · ผู้ให้บริการ · ทักษะ · ตั้งค่า"]
...
| `npm run test` | vitest โหมด watch หนึ่ง watcher ต่อหนึ่งเวิร์กสเปซ — ควรรันภายในเวิร์กสเปซเดียว |
...
|---|---|---|
| [Next.js](https://nextjs.org) | 16.3.3 | App Router, route handler, การบิลด์ |
| [React](https://react.dev) | 19.2.8 | ส่วนติดต่อผู้ใช้ |
| [vitest](https://vitest.dev) | 4.1.11 | ตัวรันเทสต์ |
| [zustand](https://zustand.docs.pmnd.rs) | 5.0.15 | สถานะฝั่งไคลเอนต์ |
| [next-intl](https://next-intl.dev) | 4.14.1 | การแปลภาษา |
| [@sentry/nextjs](https://docs.sentry.io/platforms/javascript/guides/nextjs/) | 10.72.0 | รายงานข้อผิดพลาด ไม่ทำงานถ้าไม่มี DSN |

เวอร์ชันที่แน่นอนของทุกไลบรารีถูกตรึงไว้ใน `package-lock.json`
...
> [!IMPORTANT]
> ชุดทดสอบกว่าสามสิบชุด resolve contract fixture ที่อยู่ใต้ `shared/` โดยอิงกับไดเรกทอรีทำงาน
> ดังนั้น **เทสต์จะผ่านก็ต่อเมื่อ checkout มาทั้งที่เก็บโค้ด และรันจากเวิร์กสเปซที่เป็นเจ้าของมัน**
> — คัดลอกเฉพาะ `web/` ออกไปจะไม่ทำงาน
