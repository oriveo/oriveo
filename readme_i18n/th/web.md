...
ต้องใช้ Node 22 (ดู [`.nvmrc`](../../web/.nvmrc)) npm มาพร้อมกับ Node อยู่แล้ว
...
นี่คือส่วนที่ควรอ่านก่อนอย่างอื่น เพราะไคลเอนต์เว็บเป็นที่เดียวที่คำขอ **ไม่ได้**
...
flowchart LR
    ...
    browser ==>|"ผู้ให้บริการทางการ"| chat ==> official
    browser ==>|"relay · โฮสต์สาธารณะ"| fwd ==> pubrelay
    browser ==>|"relay · LAN หรือ localhost"| lan
    catalog -.-> browser
    catalog -.-> chat
```

**ทำไมต้องอ้อม** API ของผู้ให้บริการไม่ส่ง CORS header เบราว์เซอร์จึงเรียก `api.openai.com`
และเพื่อน ๆ ตรง ๆ ไม่ได้ — preflight ล้มเหลว ไคลเอนต์ BYOK บนเบราว์เซอร์ทุกตัวต้องแก้ปัญหานี้
ด้วยวิธีใดวิธีหนึ่ง ตัวนี้เลือกส่งต่อผ่าน Next.js route handler ที่รันบนรันไทม์ Node
เมื่อคุณรัน `npm run dev:app` handler นั้นอยู่บนเครื่องของคุณเอง
เมื่อคุณดีพลอยแอปไปที่ไหนสักแห่ง มันก็อยู่บนเครื่องที่คุณดีพลอยไป

**สิ่งที่ handler ทำและไม่ทำ** มันตรวจสอบรูปแบบของคำขอและจำกัดขนาด บังคับ rate limit รายไอพี
ปฏิเสธ URL ที่ resolve ไปยังที่อยู่ส่วนตัวหรือ link-local ประกอบ body เฉพาะของผู้ให้บริการ
แล้วสตรีมคำตอบกลับมา มันไม่เก็บคีย์ ข้อความ หรืออะไรก็ตามที่ได้มาจากสิ่งเหล่านั้นไว้เลย
และมีเทสต์เฉพาะทาง (`server-never-learns.test.ts`) ตรึงพฤติกรรมนั้นไว้ ส่วนตัวส่งต่อของ relay
ยังตรึง DNS ไว้กับที่อยู่ที่มัน resolve ได้ จำกัดขนาดคำตอบ กำหนดขอบเขตของทุก timeout
จำกัดการ redirect ให้อยู่ใน origin เดิม และปฏิเสธการส่งผ่าน hop-by-hop header
...
`fetch`, `crypto`, `localStorage` และ `indexedDB` ภายในนั้น
อะไรก็ตามที่มันต้องการจากสภาพแวดล้อมจะเข้ามาผ่าน `CorePorts` นั่นคือเหตุผลที่โค้ดชุดเดียวกัน
รันได้ทั้งในเบราว์เซอร์ ใน Node route handler และในเทสต์ที่ไม่มี DOM
...
| ค่าตั้งค่าและตารางตัวควบคุมโมเดล | `localStorage` ผ่าน wrapper ที่ไม่มีวัน throw เสมอ |
...
แล้วตั้งเฉพาะที่คุณต้องการ แต่ละคีย์มีคำอธิบายอยู่ในไฟล์นั้นแล้ว

## การทดสอบ

เทสต์ราว 4,600 รายการ กระจายอยู่ใน 461 ไฟล์ ด้วย vitest ความครอบคลุมหนาแน่นที่สุด
...
> [!IMPORTANT]
> ชุดทดสอบราว 24 ชุดโหลด contract fixture จาก `../shared` ดังนั้น
