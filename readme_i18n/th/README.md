...
ไม่มีบัญชี ไม่มีการสมัครสมาชิก และไม่มีเซิร์ฟเวอร์ของเราคั่นกลางระหว่างคุณกับโมเดล
...
ไม่มีบัญชี Oriveo ไม่มีการสมัครสมาชิก และไม่มีการเก็บข้อมูลวิเคราะห์การใช้งาน

แอปคุยกับ **ผู้ให้บริการโมเดล 15 ราย** ได้แบบเนทีฟ — OpenAI, Anthropic, Google Gemini, OpenRouter,
DeepSeek, Grok, Mistral, Groq, Together AI, Fireworks AI, MiniMax, Z.ai, Qwen, Kimi และ SiliconFlow
— และยังชี้ไปที่ **ปลายทางใดก็ได้ที่เข้ากันได้กับ OpenAI, Anthropic หรือ Gemini** รวมถึง llama.cpp,
Ollama, LM Studio หรือ vLLM ที่รันอยู่บนเครื่องของคุณเอง
...
| **การเรียกที่แอปทำในนามของตัวเอง** | หนึ่งรายการ: แคตตาล็อกโมเดลแบบอ่านอย่างเดียว ไม่แนบคีย์และไม่แนบตัวระบุตัวตน |
| **สัญญาอนุญาต** | AGPL-3.0-or-later |

## ทำไมถึงมีโปรเจกต์นี้

แอปแชทไม่ควรมาขวางระหว่างคุณกับโมเดลที่คุณจ่ายเงินใช้อยู่
...
- **พฤติกรรมเดียว ไคลเอนต์สามตัว** วิธีประกอบคำขอสำหรับผู้ให้บริการ transport และความสามารถหนึ่ง ๆ
  ถูกนิยามไว้ครั้งเดียวใน [`shared/`](shared.md) และไคลเอนต์ทั้งสามตรวจสอบกับ JSON fixture
  ชุดเดียวกัน พฤติกรรมประหลาดของผู้ให้บริการจึงแก้ครั้งเดียว ไม่ใช่สามครั้ง
- **ตรงไปตรงมาเรื่องคำขอเดียวที่แอปส่งเอง** แอปดึงแคตตาล็อกโมเดลสาธารณะมา
  เพื่อให้โมเดลที่เพิ่งออกวันนี้ใช้งานได้โดยไม่ต้องอัปเดตแอป มันเป็นแบบอ่านอย่างเดียว
  ไม่แนบคีย์และไม่แนบตัวระบุตัวตน และคุณจะชี้ไปที่โฮสต์ของคุณเองก็ได้
...
- **เซิร์ฟเวอร์โมเดลในเครื่อง** — llama.cpp, Ollama, LM Studio, vLLM
  พร้อมการค้นหาอัตโนมัติในเครือข่ายท้องถิ่น
...
| โน้ต โฟลเดอร์ สกิล ไฟล์แนบ | ได้ ไม่จำกัด | ได้ |
...
| ข้อมูลวิเคราะห์และรายงานแครช | ไม่มี | มี |

บิลด์ของ Community Edition ใช้คำนำหน้าตัวระบุ `ai.oriveo.community` จึงติดตั้งอยู่ข้าง ๆ
บิลด์จากสโตร์ได้ โดยทั้งสองตัวไม่แชร์ keychain ช่องอัปเดต หรือข้อมูลในเครื่องกัน
...
มีความไม่สมมาตรอยู่หนึ่งจุดที่ควรรู้ นั่นคือไคลเอนต์เว็บ API ของผู้ให้บริการไม่ส่ง CORS header
เบราว์เซอร์จึงเรียกตรงไม่ได้ คำขอที่ไปยังผู้ให้บริการทางการทั้ง 15 ราย
จึงต้องผ่าน Next.js route handler ที่รันอยู่บนเครื่องซึ่งให้บริการแอป —
ซึ่งก็คือเครื่องของคุณเองเมื่อคุณรันในเครื่อง ไคลเอนต์ iOS และ Android ไม่มีข้อจำกัดนี้
จึงยิงตรงไปหาผู้ให้บริการ ส่วนปลายทาง relay ที่อยู่ในเครือข่ายของคุณเอง
ก็ถูกเรียกตรงจากเบราว์เซอร์เช่นกัน
...
ต้องมี JDK 17 ขึ้นไปและ Android SDK บิลด์นี้ใช้ AGP 9.3, Gradle 9.5 และ Kotlin 2.3
...
- **คีย์ของผู้ให้บริการ** ถูกเก็บด้วยกลไกของแพลตฟอร์มเอง — iOS Keychain, Android Keystore
  (`EncryptedSharedPreferences`) หรือ IndexedDB ของเบราว์เซอร์ — และถูกใช้เพื่อเข้าถึง
  ผู้ให้บริการที่เป็นเจ้าของคีย์นั้นเท่านั้น บนเว็บคีย์ถูกเก็บแบบไม่เข้ารหัส
...
- **ไม่มีบัญชี ไม่มีข้อมูลวิเคราะห์ ไม่มีรายงานแครช** ไม่มีอะไรให้เข้าสู่ระบบ
  และไม่มีอะไรแอบส่งข้อมูลกลับบ้าน
- **บน iOS และ Android คำขอแชทวิ่งตรงจากอุปกรณ์ไปหาผู้ให้บริการ** ส่วนบนเว็บ คำขอจะผ่าน
  เซิร์ฟเวอร์ Next.js ที่ให้บริการแอปอยู่ เพราะ API ของผู้ให้บริการไม่อนุญาตให้เบราว์เซอร์เรียกตรง
...
คำขอจะผ่านเซิร์ฟเวอร์ Next.js ที่ให้บริการแอปอยู่ — ซึ่งก็คือเครื่องของคุณเองเมื่อคุณรันในเครื่อง
เพราะเบราว์เซอร์เรียก API ของผู้ให้บริการตรง ๆ ไม่ได้ ทั้งสองเส้นทางไม่มีเซิร์ฟเวอร์ที่ Oriveo
ดำเนินการอยู่เลย คำขอเดียวที่ Oriveo ส่งในนามของตัวเอง คือการดึงแคตตาล็อกโมเดลสาธารณะ
แบบอ่านอย่างเดียว ซึ่งไม่แนบคีย์ ไม่แนบบทสนทนา และไม่แนบตัวระบุตัวตน
...
ไคลเอนต์ Android และเว็บยังค้นหาเซิร์ฟเวอร์แบบนั้นในเครือข่ายท้องถิ่นได้ด้วย HTTP ในเครือข่ายท้องถิ่น
ไม่ใช้ข้อมูลรับรองใด ๆ และไม่ออกไปนอกเครือข่ายของคุณ
...
ไม่มีข้อมูลวิเคราะห์ ดูตารางเทียบแบบเต็มได้ที่
...
และบิลด์ iOS ก็รันบน Mac ที่ใช้ Apple silicon ได้
