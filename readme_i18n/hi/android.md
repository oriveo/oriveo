...
हैं, और ऐप हर प्रोवाइडर से सीधे फ़ोन से बात करता है। बातचीत, नोट्स, फ़ोल्डर और कौशल डिवाइस पर Room
...
flowchart TB
    subgraph ui ["Compose UI"]
        ...
        screens["चैट · होम · प्रोवाइडर<br/>नोट्स · कौशल · सेटिंग्स"]
    end
...
**दो डेटाबेस, एक नहीं।** `oriveo.db` में बातचीत, मैसेज, अटैचमेंट, नोट्स, फ़ोल्डर, कौशल और मॉडल-कैटलॉग
...
| क्या | कहाँ |
|---|---|
| बातचीत, मैसेज, अटैचमेंट, नोट्स, फ़ोल्डर, कौशल | Room, `oriveo.db` |
...
PBKDF2-HMAC-SHA256 और AES-GCM से एन्क्रिप्ट होकर `data.json` के एक फ़ील्ड के रूप में रखी जाती हैं।
बातचीत, मैसेज, नोट्स, फ़ोल्डर, कौशल, प्राथमिकताएँ और अटैचमेंट हर हाल में सादे JSON और सादी फ़ाइलों
के रूप में लिखे जाते हैं, इसलिए आर्काइव को ऐसा ही मानें कि जिसके पास वह फ़ाइल है, वह उसे पढ़ सकता है।
अगर आपको सिर्फ़ अपना इतिहास चाहिए तो key के बिना एक्सपोर्ट करें।
...
> - रिले सेवा (Relay) के endpoint और लोकल मॉडल सर्वर पूरी तरह काम करते रहते हैं, और वही इकलौता सही-सलामत रास्ता हैं।
>
...
318 फ़ाइलों में क़रीब 3,000 unit टेस्ट, जो JUnit 4, MockK, Robolectric, `kotlinx-coroutines-test` और
Ktor के mock engine का इस्तेमाल करते हैं। कवरेज वहाँ सबसे घनी है जहाँ ग़लतियाँ सबसे महँगी पड़ती हैं: हर
...
> [!IMPORTANT]
> क़रीब 38 suites, working directory से ऊपर की ओर चलकर `shared/` ढूँढ़ती हैं और वहीं से contract
> fixtures लोड करती हैं, इसलिए **टेस्ट सिर्फ़ पूरे checkout में ही पास होते हैं** — अकेले `android/`
> को कॉपी करके ले जाना काम नहीं करेगा।
